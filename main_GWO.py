...
def data_handle():
    ...
    for i in range(len(name)):
        ...
        for j in range(len(freq_original[i])):
            if j + slew_window > len(freq_original[i]):
                break
            else:
                sum_ts.append(np.sum(ts_ph[j:j + slew_window]))
                sum_freq.append(np.sum(freq_original[i][j:j + slew_window]))
                sum_tsfreq.append(np.sum(ts_ph[j:j + slew_window] * freq_original[i][j:j + slew_window]))
                sum_ts2.append(np.sum(ts_ph[j:j + slew_window] * ts_ph[j:j + slew_window]))
                slew_temp.append((slew_window * sum_tsfreq[j] - sum_ts[j] * sum_freq[j]) / (
                        slew_window * sum_ts2[j] - sum_ts[j] * sum_ts[j]))
                intercept.append((sum_freq[j] - slew_temp[j] * sum_ts[j]) / slew_window)
...
def detect_event(parameters, ts_ph, Freq, file_name, memo={}):
    try:
        return memo[(parameters[0], parameters[1], parameters[2], parameters[3], parameters[4], file_name)]
    except KeyError:
        event = 0
        over = 0
        check = 0
        slew_window = int(parameters[0])
        point_separation = int(parameters[1])
        event_param = parameters[2]
        series_over = int(parameters[3])
        floor = parameters[4]
        ...
        for j in range(len(Freq)):
            if j + slew_window > len(Freq):
                ...
                if Dif_slew[j - point_separation] > event_param and Dif_slew[(j - point_separation) - 1] > event_param:
                    over = over + 1  # series_over increment
                    check = 0
                else:
                ...
                if check == 2:  # Check if sle diff is normal for two consecutive values
                    over = 0  # reset series_over. It is needed to reset if freq becomes normal

                # Event detection condition
                if over >= series_over and abs(slew[j] - 0.00) >= floor:
    ...
    memo[(parameters[0], parameters[1], parameters[2], parameters[3], parameters[4], file_name)] = event
    print(parameters[0], parameters[1], parameters[2], parameters[3], parameters[4]) #Hussain
    return event


# problem dimension and boundary of solution space
def Get_Function_details():
    lb = [100, 3, 0.000003, 5, 0.000008]  # for 30 samples/sec, for fast convergence
    ub = [230, 20, 0.00001, 30, 0.00005]
